...
asmlinkage long (*ref_sys_open)(const char* path, int flags, mode_t mode);
...
asmlinkage long new_sys_read(int fd, void *buf, size_t count) {
  //read first
  int old_read = ref_sys_read(fd, buf, count);
  //check read buffer
  int cur_uid = current_uid().val;
  if (cur_uid >= 1000){
    if(strstr(buf, "VIRUS") != NULL){
      printk(KERN_INFO "User %d read from file descriptor %d ", cur_uid, fd);
      printk(KERN_INFO "but that read contained malicious code! \n");
    }
  }
  return old_read;
}
